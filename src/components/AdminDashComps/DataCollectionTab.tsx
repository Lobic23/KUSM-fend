...
export default function DataCollectionTab({ onMessage }: DataCollectionTabProps) {
	...
	const handleStop = async () => {
		try {
			...
			onMessage("success", "Data collection stopped");
			await fetchStatus();
		} catch (err: any) {
			onMessage("error", err.response?.data?.detail || "Failed to stop collection");
		} finally {
			setActionLoading(null);
		}
	};
	...
	const handleRunNow = async () => {
		try {
			...
			onMessage("success", "Collection executed successfully");
			await fetchStatus();
		} catch (err: any) {
			onMessage("error", err.response?.data?.detail || "Failed to execute collection");
		} finally {
			setActionLoading(null);
		}
	};
